        ...
        return $schema
            ->components([
                ...
                Section::make('Daftar Barang Pinjaman')
                    ->description('Tambahkan barang yang akan dipinjam beserta jumlah dan kondisinya')
                    ->columnSpanFull()
                            ...
                            ->schema(fn (Schema $innerSchema) => $innerSchema->components([
                                Grid::make(5)->schema([
                                    Select::make('id_barang')
                                        ...
                                        ->afterStateUpdated(fn ($state, callable $set) => $set('id_stok_barang', null))
                                        ->columnSpan(2),
                                    ...
                                    Select::make('kondisi')
                                        ...
                                        ->afterStateUpdated(function ($state, callable $get, callable $set) {
                                            ...
                                            if ($barangId && $state) {
                                                $stok = StokBarang::where('id_barang', $barangId)
                                                    ->where('kondisi', $state)
                                                    ->first();
                                                if ($stok) {
                                                    $set('id_stok_barang', $stok->id);
                                                    $set('stok_tersedia', $stok->jumlah);
                                                } else {
                                    ...
                                    TextInput::make('stok_tersedia')
                                        ...
                                        ->live()
                                        ->suffix('unit')
                                        ->helperText('Stok yang tersedia saat ini'),
                                    ...
                                    TextInput::make('jumlah')
                                        ...
                                        ->required()
                                        ->minValue(1)
                                        ->maxValue(fn (callable $get) => (int) $get('stok_tersedia') ?: 1)
                                        ->suffix('unit')
                                        ->helperText('Maks: sesuai stok tersedia'),
                                ]),
                            ]))
